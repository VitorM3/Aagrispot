import { calculateNewMatrixSumOrSubtraction } from "../function/calculateMatrixSum";
...
export class MatrixService {
    ...
    async create(predefinitionsMatrix: IBodyCreate): Promise<IMatrixBase[]> {
        try {
            ...
            let matrize: IMatrixBase[] = []

            matrize = await createMatrix(matrize, numberLines, numberColumns, newArrayCalc)
        ...
        try {
            ...
            switch (baseToCreateANewMatrix.sinalOperation) {
                case "+":
                    ...
                    newMatrix = await calculateNewMatrixSumOrSubtraction(baseToCreateANewMatrix.matrixOne, baseToCreateANewMatrix.matrixTwo, baseToCreateANewMatrix.sinalOperation)
                    return newMatrix
                case "-":
                    if (numberMaxColumnOne != numberMaxColumnTwo && numberMaxLineOne != numberMaxLineTwo) {
                        const errorClient: IErrorClient = { messageErrorClient: 'Arrays must have the same order', status: 406 }; return errorClient
