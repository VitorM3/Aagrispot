...
const VariablesToCreateMatrix = () => {
    ...
    const handleClickInCalculate = async (variables:IBodyRouteCreate) => {
        if(variables.numLines > 10){
            toast.warning('Número de colunas muito grande para ser criada')
            return
        }
        if(variables.numLines > 10){
            toast.warning('Número de linhas muito grande para ser criada')
            return
        }
        // Verificar se todos os campos foram preenchidos
    ...
    return (
        <VariablesToCreateMatrixGeneral>
            <Variables>
                ...
                <ButtonCalc onClick={()=>{handleClickInCalculate({calc,numColumn,numLines})}}>Criar</ButtonCalc>
            </Variables>
        </VariablesToCreateMatrixGeneral>
    );
}
